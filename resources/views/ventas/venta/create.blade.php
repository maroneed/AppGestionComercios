 ...
 <div class="row">
  <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
   ...
   {!!Form::open(array('url'=>'ventas/venta','method'=>'POST','autocomplete'=>'off'))!!}
            ...
            <div class="row">
             ...
             <select name="clienteID" id="idcliente" class="form-control selectpicker" data-live-search="true">
            ...
            <select name="articuloID" id="pidarticulo" class="form-control selectpicker" data-live-search="true">
            @foreach($articulos as $articulo)
            <option value="{{$articulo->id}}_{{$articulo->stock}}_{{$articulo->precio_promedio}}">{{$articulo->articulo}}</option>
            @endforeach
            </select>
            </div>
            </div>
            ...
            <th>Total con iva <h4 id="total_iva">$/ 0.00</h4><input type="hidden" name="" id=""></th>
            <th>Total <h4 id="total">$/ 0.00</h4><input type="hidden" name="total_venta" id="total_venta"></th>
            <th>Iva<h4 id="iva">$/ 0.00</h4><input type="hidden" name="" id=""></th>
  ...
  function agregar(){
    ...
    datosArticulo = document.getElementById('pidarticulo').value.split('_');
        ...
        if (articuloID!="" && cantidad!="" && cantidad>0 && precio_venta!="" && descuento!="")
        {
            if (stock>=cantidad)
            {
                ...
                iva = total * 21/100;
                total_iva = total+iva;
                var fila = '<tr class="selected" id="fila'+cont+'"><td><button type="button" class="btn btn-warning" onclick="eliminar('+cont+');">X</button></td><td><input type="hidden" name="articuloID[]" value="'+articuloID+'">'+articulo+'</td><td><input type="number" name="cantidad[]" value="'+cantidad+'"></td><td><input type="number" name="precio_venta[]" value="'+precio_venta+'"></td><td><input type="number" name="descuento[]" value="'+descuento+'"></td><td>'+'$/.'+subtotal[cont]+'</td></tr>';
                cont++;
                limpiar();
